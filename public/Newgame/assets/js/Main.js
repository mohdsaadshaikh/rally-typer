...
let initTrackEmitter = function () {
  ...
  trackEmitter.update = function () {
    ...
    if (trackEmitter.vertIndex * size >= trackEmitter.numOfVerts - size) trackEmitter.vertIndex = 0;
  };
};
...
let loadModels = function () {
  gltfLoader.load("assets/models/grass-track.glb", function (gltf) {
    ...
    gltf.scene.traverse(function (node) {
      ...
      if (node.name.includes("Track") || node.name.includes("Finish_Line") || node.name.includes("Start_Line")) {
    ...
    grass.material.onBeforeCompile = function (shader) {
      ...
      const customColor = `
			...
			`;
      shader.fragmentShader = shader.fragmentShader.replace(token, customColor);
      grassShader = shader;
    };
...
let loadCar2 = function (animations) {
  gltfLoader.load("assets/models/car.glb", function (gltf) {
    ...
    gltf.scene.traverse(function (node) {
      ...
      if (
        node.material &&
        (node.material.name.includes("Lacquer_Mat") ||
          node.material.name.includes("Glass_Mat") ||
          node.material.name.includes("Silver_Mat") ||
          node.material.name.includes("Tire_Mat"))
      ) {
        node.material.setValues({
          side: THREE.DoubleSide,
        });
  ...
  window.addEventListener(
    ...
    function (evt) {
      ...
      if (evt.key === TEXT[index]) {
        ...
        else car2.animations.drive.timeScale += STARTING_BONUSES[index];
        ...
        if (gameState.correct) {
          ...
          if (car2.animations.drive.timeScale > MINIMUM_SPEED) car2.animations.drive.timeScale -= MISTAKE_DECREMENT;
        }
...
let initTextures = function () {
  ...
  Textures.skyBox = cubeTextureLoader.load(["px" + format, "nx" + format, "py" + format, "ny" + format, "pz" + format, "nz" + format], function () {
...
let initLights = function () {
  ...
  Lights[0] = new THREE.HemisphereLight(0xffffff, 0xa0a0a0, 1.0);
...
let updateShadowCamera = function () {
  ...
  Lights[1].position.set(100 + car2.position.x, 100, -40 + car2.position.z);
...
function animate() {
  ...
  time += 0.1;
  if (flagShader) flagShader.uniforms.uTime.value = time / 2;
  if (grassShader) grassShader.uniforms.uTime.value = time * 2.0;
  ...
  if (!gameState.loadingAssets) {
    ...
    car2.temp.setFromMatrixPosition(car2.goal.matrixWorld);
    if (gameState.START == false || car2.animations.drive.timeScale < 0.1) car2.alpha = 1.2;
    else car2.alpha = car2.animations.drive.timeScale * 7.5; // 1 - 3.25 (0.5 tScale)
